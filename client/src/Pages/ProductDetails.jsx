...
const ProductDetails = () => {
    ...
    const fetchProduct = async () => {
      try {
        const res = await fetch(
          `https://e-commerce-backend-2isg.onrender.com/api/home/all-products/${id}`
        );
        if (!res.ok) {
        ...
        if (data.category) {
          try {
            const relatedRes = await fetch(
              `https://e-commerce-backend-2isg.onrender.com/api/home/products?category=${data.category}`
            );
            if (!relatedRes.ok) {
              throw new Error(
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const res = await fetch("https://e-commerce-backend-2isg.onrender.com/api/user/", {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      const res = await fetch("https://e-commerce-backend-2isg.onrender.com/api/order/create", {
        method: "POST",
        headers: {
